...
const EditModal = ({ setOpenEditModal, userList, currentUser, setUserList }) => {
    ...
    return <div className='w-[25%] m-auto p-12 bg-white  rounded'>
        <div className="flex justify-between">
            <h1 className='text-2xl font-bold text-black mb-[20px] text-center'> Update</h1>
        ...
        <form className='w-full' onSubmit={onSubmitForm} >
            ...
            {loadingIcon(loading)}
            {!updateUser.name ? <span className='text-red'>{error}</span> : (!updateUser.age || isNaN(updateUser.age)) && <span className='text-red'>{error}</span>}
        </form>
    </div>
}
